...
from langchain_classic.chains import RetrievalQA     # ✅ FIX #2: langchain_classic no existe
...
app = FastAPI(title="Unified Document API", version="1.1")
...
async def index_folder(folder_path: str = Form(...)):
    ...
    try:
        ...
        docs = RecursiveCharacterTextSplitter(chunk_size=1200, chunk_overlap=200).split_documents(raw_documents)
...
async def ask_folder(question: str = Form(...)):
    ...
    chain = RetrievalQA.from_chain_type(
        # ✅ FIX #1: api_key usa la variable de entorno, no hardcodeada sin comillas
        llm=ChatGroq(model="llama-3.1-8b-instant", api_key=GROQ_API_KEY, temperature=0.1),
        retriever=folder_vectorstore.as_retriever(search_type="mmr", search_kwargs={'k': 6}),
...
@app.get("/health")
async def health():
    return {"pdf_ready": pdf_vectorstore is not None, "folder_ready": folder_vectorstore is not None}
